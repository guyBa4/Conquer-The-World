...
public class GameServiceTest {
    ...
    public void testAddGameInstance() {
        ...
        List<Object> startingPositions = new ArrayList<>();
        startingPositions.add(UUID.randomUUID().toString());
        ...
        Response<GameInstance> response = gameService.addGameInstance("Test Game", "Test Description", questionnaireUuid, mapUuid, creatorUuid, 2, 60, true, 30, startingPositions, true, true, true, new ArrayList<>());
    ...
    @Test
    public void testAddGameInstanceWithInvalidMap() {
        UUID questionnaireUuid = UUID.randomUUID();
        UUID mapUuid = UUID.randomUUID();
        UUID creatorUuid = UUID.randomUUID();
        List<Object> startingPositions = new ArrayList<>();
        ...
        when(dalController.getQuestionnaire(questionnaireUuid)).thenReturn(mock(Questionnaire.class));
        when(dalController.getMap(mapUuid)).thenThrow(new IllegalArgumentException("Invalid Map"));

        Response<GameInstance> response = gameService.addGameInstance("Test Game", "Test Description", questionnaireUuid, mapUuid, creatorUuid, 2, 60, true, 30, startingPositions, true, true, true, new ArrayList<>());

        assertFalse(response.isSuccessful(), "Expected response to be unsuccessful");
        assertEquals(403, response.getStatus(), "Expected status to be 403");
        assertEquals("Invalid Map", response.getMessage(), "Expected message to be 'Invalid Map'");
    }

    @Test
    public void testAddGameInstanceWithInvalidUser() {
        UUID questionnaireUuid = UUID.randomUUID();
        UUID mapUuid = UUID.randomUUID();
        UUID creatorUuid = UUID.randomUUID();
        List<Object> startingPositions = new ArrayList<>();
        ...
        when(dalController.getQuestionnaire(questionnaireUuid)).thenReturn(mock(Questionnaire.class));
        when(dalController.getMap(mapUuid)).thenReturn(mock(GameMap.class));
        when(dalController.getUser(creatorUuid)).thenThrow(new IllegalArgumentException("Invalid User"));

        Response<GameInstance> response = gameService.addGameInstance("Test Game", "Test Description", questionnaireUuid, mapUuid, creatorUuid, 2, 60, true, 30, startingPositions, true, true, true, new ArrayList<>());

        assertFalse(response.isSuccessful(), "Expected response to be unsuccessful");
        assertEquals(403, response.getStatus(), "Expected status to be 403");
        assertEquals("Invalid User", response.getMessage(), "Expected message to be 'Invalid User'");
    }

    @Test
    public void testDeleteGameWithRunningInstances() {
        UUID gameId = UUID.randomUUID();
        List<RunningGameInstance> runningGameInstances = Arrays.asList(mock(RunningGameInstance.class), mock(RunningGameInstance.class));
    ...
    @Test
    public void testDeleteGameWithInvalidIdAndRunningInstances() {
        UUID gameId = UUID.randomUUID();
